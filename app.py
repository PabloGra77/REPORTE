import pandas as pd
import streamlit as st
import plotly.express as px
import plotly.graph_objects as go
from io import BytesIO

# ==========================
# CONFIGURACIÓN GENERAL
# ==========================
st.set_page_config(page_title="Panel GIA", page_icon="🤖", layout="wide")

# Encabezado con estilo corporativo
st.markdown("""
<style>
    .title {
        text-align: center;
        font-size: 40px;
        font-weight: bold;
        color: #3A86FF;
    }
    .subtitle {
        text-align: center;
        color: #FF9F1C;
        font-size: 18px;
        margin-bottom: 30px;
    }
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="title">🤖 Panel de Estadísticas GIA</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">IPS Goleman | Plataforma GIA - Inteligencia para el Soporte</div>', unsafe_allow_html=True)

st.markdown("""
Sube tu archivo **Excel (.xlsx)** o **CSV (.csv)** exportado del sistema **GIA**  
para generar reportes mensuales o quincenales.  
> Nota: La columna **“Cantidad de casos abiertos”** se trata como **“Casos asignados”** para los cálculos.
""")

# ==========================
# SUBIR ARCHIVO
# ==========================
uploaded_file = st.file_uploader("📁 Cargar archivo Excel o CSV", type=["xlsx", "csv"])

# ==========================
# PROCESAR DATOS
# ==========================
if uploaded_file:
    try:
        # Detectar tipo de archivo
        if uploaded_file.name.endswith(".csv"):
            df = pd.read_csv(uploaded_file, sep=None, engine="python")
        else:
            df = pd.read_excel(uploaded_file)

        df = df.fillna(0)

        # ----- Nombres de columnas esperadas -----
        COL_TECNICO   = df.columns[0]  # primera columna = nombre del técnico/grupo
        COL_ABIERTOS_ORIG = "Cantidad de casos abiertos"     # viene del export
        COL_ASIGNADOS = "Casos asignados"                    # alias interno correcto
        COL_RESUELTOS = "Cantidad de casos resueltos"
        COL_TARDIOS   = "Cantidad de casos tardíos"

        # Si existe la columna original, crear alias "Casos asignados"
        if COL_ABIERTOS_ORIG in df.columns:
            df[COL_ASIGNADOS] = df[COL_ABIERTOS_ORIG]
        elif COL_ASIGNADOS not in df.columns:
            # Si no viene ninguna, crea la columna para evitar errores
            df[COL_ASIGNADOS] = 0

        # Columnas que NO deben afectar estadísticas
        columnas_ignorar = {
            "Cantidad de casos cerrados",
            "Cantidad de encuestas de satisfacción abiertas",
            "Cantidad de encuestas de satisfacción respuestas",
            "Satisfacción promedio"
        }

        # Asegurar técnico como texto
        df.iloc[:, 0] = df.iloc[:, 0].astype(str).str.strip()

        # Quitar filas sin técnico válido
        df = df[~df.iloc[:, 0].isin(["", "0", "nan", "None"])].reset_index(drop=True)

        # Convertir a numérico solo columnas relevantes
        columnas_relevantes = [c for c in df.columns if c not in columnas_ignorar and c != COL_TECNICO]
        for col in columnas_relevantes:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

        # ===== CÁLCULOS =====
        # Eficiencia = resueltos / asignados
        df["Eficiencia (%)"] = (df[COL_RESUELTOS] / (df[COL_ASIGNADOS] + 1e-9)) * 100

        # Cumplimiento SLA = (resueltos - tardíos) / resueltos
        df["Cumplimiento SLA (%)"] = ((df[COL_RESUELTOS] - df[COL_TARDIOS]) / (df[COL_RESUELTOS] + 1e-9)) * 100

        # Técnicos activos (algún valor > 0 en asignados, resueltos o tardíos)
        df_validos = df[(df[COL_ASIGNADOS] > 0) | (df[COL_RESUELTOS] > 0) | (df[COL_TARDIOS] > 0)].copy()

        # ==========================
        # TABLA Y RESUMEN
        # ==========================
        st.subheader("📋 Tabla de resultados (con alias: “Casos asignados”)")
        st.dataframe(df_validos, use_container_width=True)

        eficiencia_prom = round(df_validos["Eficiencia (%)"].mean(), 2) if len(df_validos) else 0.0
        sla_prom = round(df_validos["Cumplimiento SLA (%)"].mean(), 2) if len(df_validos) else 0.0

        if len(df_validos):
            mejor_tecnico = df_validos.iloc[df_validos["Eficiencia (%)"].idxmax(), 0]
            peor_tecnico  = df_validos.iloc[df_validos["Eficiencia (%)"].idxmin(), 0]
        else:
            mejor_tecnico = "—"
            peor_tecnico  = "—"

        st.markdown(f"""
        ### 📊 Resumen General
        - 🧩 **Eficiencia promedio:** {eficiencia_prom}%
        - ⏱️ **Cumplimiento SLA promedio:** {sla_prom}%
        - 🥇 **Mejor técnico:** {mejor_tecnico}
        - 🧰 **Técnico con menor eficiencia:** {peor_tecnico}
        """)

        # ==========================
        # GRÁFICOS INTERACTIVOS
        # ==========================
        st.subheader("📊 Comparativo por Técnico (Efecto 3D Visual)")
        fig1 = go.Figure()
        fig1.add_trace(go.Bar(
            x=df_validos[COL_TECNICO], y=df_validos[COL_ASIGNADOS],
            name="Casos Asignados",
            marker=dict(color="rgba(58,134,255,0.8)", line=dict(color="rgba(58,134,255,1)", width=2)),
        ))
        fig1.add_trace(go.Bar(
            x=df_validos[COL_TECNICO], y=df_validos[COL_RESUELTOS],
            name="Casos Resueltos",
            marker=dict(color="rgba(255,159,28,0.8)", line=dict(color="rgba(255,159,28,1)", width=2)),
        ))
        fig1.add_trace(go.Bar(
            x=df_validos[COL_TECNICO], y=df_validos[COL_TARDIOS],
            name="Casos Tardíos",
            marker=dict(color="rgba(255,70,70,0.8)", line=dict(color="rgba(255,70,70,1)", width=2)),
        ))
        fig1.update_layout(
            barmode='group',
            title="Asignados vs Resueltos vs Tardíos - GIA",
            xaxis_title="Técnico",
            yaxis_title="Cantidad",
            template="plotly_dark",
            bargap=0.2,
            plot_bgcolor="rgba(20,20,30,1)",
            paper_bgcolor="rgba(10,10,15,1)"
        )
        st.plotly_chart(fig1, use_container_width=True)

        st.subheader("💪 Eficiencia por Técnico (%)")
        fig2 = px.bar(
            df_validos, x=COL_TECNICO, y="Eficiencia (%)",
            text_auto=".2f", color="Eficiencia (%)",
            color_continuous_scale="Blues", title="Eficiencia (Resueltos / Asignados)"
        )
        fig2.update_traces(marker_line_width=1.3)
        fig2.update_layout(template="plotly_dark", bargap=0.3)
        st.plotly_chart(fig2, use_container_width=True)

        st.subheader("⏱️ Cumplimiento SLA por Técnico (%)")
        fig3 = px.bar(
            df_validos, x=COL_TECNICO, y="Cumplimiento SLA (%)",
            text_auto=".2f", color="Cumplimiento SLA (%)",
            color_continuous_scale="Oranges", title="Cumplimiento de SLA"
        )
        fig3.update_traces(marker_line_width=1.3)
        fig3.update_layout(template="plotly_dark", bargap=0.3)
        st.plotly_chart(fig3, use_container_width=True)

        # ==========================
        # DESCARGA RESULTADOS
        # ==========================
        output = BytesIO()
        df_validos.to_excel(output, index=False, engine='openpyxl')
        st.download_button(
            label="⬇️ Descargar reporte Excel con estadísticas GIA",
            data=output.getvalue(),
            file_name="reporte_estadistico_GIA.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

    except Exception as e:
        st.error(f"❌ Error al procesar el archivo: {e}")

else:
    st.info("📄 Sube un archivo Excel o CSV del sistema GIA para comenzar el análisis.")
