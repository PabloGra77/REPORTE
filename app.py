...
Los campos sin datos o con valor "0" serán ignorados para mantener estadísticas reales.  
""")
...
if uploaded_file:
    try:
        ...
        # 🔹 Eliminar filas sin técnico válido (vacías, '0', 'nan', 'None')
        ...
        # ==========================
        # LIMPIEZA: Ignorar técnicos sin actividad
        # ==========================
        df_validos = df[
            (df["Cantidad de casos abiertos"] > 0) |
            (df["Cantidad de casos resueltos"] > 0) |
            (df["Cantidad de casos tardíos"] > 0)
        ].copy()
        ...
        # Cálculo de promedios sin incluir técnicos inactivos
        ...
        # ==========================
        # GRÁFICOS INTERACTIVOS (efecto 3D visual)
        # ==========================
        st.subheader("📊 Comparativo de Casos por Técnico (Efecto 3D Visual)")

        fig1 = go.Figure()
        fig1.add_trace(go.Bar(
            x=df_validos[df_validos.columns[0]],
            y=df_validos["Cantidad de casos abiertos"],
            name="Casos Abiertos",
            marker=dict(color="rgba(58,134,255,0.8)", line=dict(color="rgba(58,134,255,1)", width=2)),
        ))
        fig1.add_trace(go.Bar(
            x=df_validos[df_validos.columns[0]],
            y=df_validos["Cantidad de casos resueltos"],
            name="Casos Resueltos",
            marker=dict(color="rgba(255,159,28,0.8)", line=dict(color="rgba(255,159,28,1)", width=2)),
        ))
        fig1.add_trace(go.Bar(
            x=df_validos[df_validos.columns[0]],
            y=df_validos["Cantidad de casos tardíos"],
            name="Casos Tardíos",
